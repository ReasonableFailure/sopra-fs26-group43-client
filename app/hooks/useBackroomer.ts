...
export function useBackroomer(scenarioId: number) {
    ...
    const { value: backroomerId, set: setBackroomerId } = useLocalStorage<number | null>(key, null);
    const { value: backroomerToken, set: setBackroomerToken } = useLocalStorage<string | null>(key, null);
