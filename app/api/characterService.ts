...
export class CharacterService {
  ...
  getCharactersByScenario(scenarioId: number, token: string): Promise<Character[]> {
    return this.api.getWithToken<Character[]>(`/characters/scenario/${scenarioId}`, token);
  }
  ...
  updateCharacter(characterId: number, dto: CharacterPutDTO, directorToken: string): Promise<void> {
    return this.api.putWithToken<void>(`/characters/${characterId}`,dto,`Director ${directorToken}`);
  }

  getCharacterById(characterId: number, token: string): Promise<Character> {
    return this.api.getWithToken<Character>(`/characters/${characterId}`, token);
  }
}
